...
public class MyCluster {
  ...
  public String getKafkaBrokersListAsString(){
    String kafkaBrokers = "";
    Iterator<HostPort> it = this.getKafkaHosts().iterator();
    while(it.hasNext()){
      kafkaBrokers += it.next().toString()+",";
    }
    return kafkaBrokers;
  }
  
  public String getZookeeperListAsString(){
    String zookeepers = "";
    Iterator<HostPort> it = this.getZkHosts().iterator();
    while(it.hasNext()){
      zookeepers += it.next().toString()+",";
    }
    return zookeepers;
  }
}
