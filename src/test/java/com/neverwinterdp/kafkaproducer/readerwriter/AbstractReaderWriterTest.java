...
public abstract class AbstractReaderWriterTest {
	...
	protected static final Logger logger = Logger
			.getLogger(TestKafkaWriter.class);
	...
	protected void initCluster(int numOfZkInstances, int numOfKafkaInstances)
			throws Exception {
	...
	private void writeAndRead() throws Exception {
		...
		KafkaWriter writer = new KafkaWriter.Builder(zkURL, topic).properties(
				props).build();
		writer.connect();
	...
	public void writeToNonExistentTopic() throws Exception {
		try {
			initCluster(1, 1);
			Properties props = initProperties();
			KafkaWriter writer = new KafkaWriter.Builder(zkURL, "someTopic")
					.properties(props).partition(99).build();
			writer.connect();
	...
	public void testWriteToWrongServer() throws Exception {
		...
		try {
			...
			Properties props = initProperties();
			Collection<HostPort> brokers = helper.getBrokersForTopic(topic)
					.values();
			killLeader(topic);
			brokers = helper.getBrokersForTopic(topic).values();
			KafkaWriter writer = new KafkaWriter.Builder(brokers, topic)
	...
	public void testCheckWritenDataExistOnPartition() throws Exception {
		try {
			...
			Properties props = initProperties();
			for (int i = 0; i < 3; i++) {
				KafkaWriter writer = new KafkaWriter.Builder(zkURL, topic)
						.properties(props).partition(i).build();
				writer.connect();
	...
	public void testWriteToSingleTopicSinglePartition() throws Exception {
		try {
			...
			Properties props = initProperties();
			KafkaWriter writer = new KafkaWriter.Builder(zkURL, topic)
					.properties(props).partition(0).build();
			writer.connect();
			writer.write("message");
			KafkaReader reader;
	...
	public void testWriteTenThousandMessagesToFiveTopics() throws Exception {
		try {
			...
			for (int i = 0; i < 5; i++) {
				...
				KafkaWriter writer = new KafkaWriter.Builder(zkURL, topics[i])
						.properties(props).partition(0).build();
				writer.connect();
	...
	@Test
	public void testWriteTenThousandMessagesToFiveTopicsTwoPartition()
			throws Exception {
		try {
			initCluster(1, 1);
			...
			for (int i = 0; i < 5; i++) {
				...
				for (int partition : partitions) {
					KafkaWriter writer = new KafkaWriter.Builder(zkURL,
							topics[i]).properties(props).partition(partition)
							.build();
					writer.connect();
	...
	public void testRetryUntilKafkaStart() throws Exception {
		...
		try {
			...
			retryer = new RunnableRetryer(new DefaultRetryStrategy(10, 3000,
	...
	public void testKillLeaderAndRebalance() throws Exception {
		...
		try {
			...
			final HostPort leader = helper.getLeaderForTopicAndPartition(topic,
					0);
			Properties props = initProperties();
			KafkaWriter.Builder builder = new KafkaWriter.Builder(zkURL, topic)
					.partition(0).properties(props);
							...
							new Thread(new Runnable() {
								...
								public void run() {
									...
									try {
										...
										for (KafkaServer server : cluster
												.getKafkaServers()) {
											remainingBrokers.add(server
													.config().brokerId());
										}
										System.err.println("Leader killed  ");
										int brokersForTopic = helper
												.getBrokersForTopicAndPartition(
														topic, 0).size();
										assertEquals(replicationFactor - 1,
												brokersForTopic);
										helper.rebalanceTopic(topic, 0,
												remainingBrokers);
			...
			RunnableRetryer retryer = new RunnableRetryer(
					new DefaultRetryStrategy(5, 500,
							FailedToSendMessageException.class), writer);
			writer.connect();
			retryer.run();
			Thread.sleep(10000);
	...
	@Test
	public void testKillLeaderRebalanceAndRestart() throws Exception {

		final int kafkaBrokers = 4;
		final int replicationFactor = 3;
		initCluster(1, kafkaBrokers);
							...
							new Thread(new Runnable() {
								...
								public void run() {
									...
									try {
										...
										System.err.println("Leader killed  ");
										// before rebalance the shouldn't be
										// equal
										int brokersForTopic = helper
												.getBrokersForTopicAndPartition(
														topic, 0).size();
										...
										helper.rebalanceTopic(topic, 0,
												remainingBrokers);
										System.err.println("rebalance done ");
										brokersForTopic = helper
												.getBrokersForTopicAndPartition(
														topic, 0).size();
										deadLeader.startup();
										cluster.getKafkaServers().add(
												deadLeader);
										System.err
												.println("dead leader started");
